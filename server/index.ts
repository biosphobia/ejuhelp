...
import { topicsFor, SUBJECTS, type Subject } from './eju';
import { hasApiKey, ask, generate, check, notes } from './claude';
...
// Topic list for a subject (no API key needed).
app.post('/api/eju/topics', (req: Request, res: Response) => {
  ...
  res.json({ topics: topicsFor(subject, toLang(lang)) });
});
...
app.post('/api/claude/generate', requireAuth, guardKey, async (req: Request, res: Response) => {
  try {
    const { subject, lang, topic, difficulty, count } = req.body ?? {};
    if (!isSubject(subject)) return res.status(400).json({ error: 'bad_subject' });
...
app.post('/api/claude/notes', requireAuth, guardKey, async (req: Request, res: Response) => {
  try {
